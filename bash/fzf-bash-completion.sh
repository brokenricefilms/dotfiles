...
_fzf_bash_completion_parse_dq() {
  ...
  if [[ "$last" == \"* ]]; then
    ...
    while true; do
      ...
      shell_start="$($_fzf_bash_completion_grep <<<"$line" -E -o '^(\\.|\$[^(]|[^$])*\$\(')"
      string_end="$($_fzf_bash_completion_grep <<<"$line" -E -o '^(\\.|[^"])*"')"
      ...
      if ((${#string_end} && (!${#shell_start} || ${#string_end} < ${#shell_start}))); then
        ...
        split="$(_fzf_bash_completion_shell_split <<<"$line")"
