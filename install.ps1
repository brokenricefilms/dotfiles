...
sudo New-Item -ItemType SymbolicLink -Path "$env:USERPROFILE\Documents\PowerShell\Microsoft.PowerShell_profile.ps1" -Target "$env:HOMEPATH\dotfiles\powershell\Microsoft.PowerShell_profile.ps1" -Force

sudo New-Item -ItemType SymbolicLink -Path "$env:USERPROFILE\.gitconfig" -Target "$env:HOMEPATH\dotfiles\git\gitconfig" -Force
sudo New-Item -ItemType SymbolicLink -Path "$env:USERPROFILE\.ssh\config" -Target "$env:HOMEPATH\dotfiles\ssh\config" -Force
...
sudo New-Item -ItemType SymbolicLink -Path "$env:USERPROFILE\AppData\Local\Packages\Microsoft.WindowsTerminal_8wekyb3d8bbwe\LocalState\settings.json" -Target "$env:HOMEPATH\dotfiles\windows_terminal_config.json" -Force

sudo New-Item -ItemType SymbolicLink -Path "$env:USERPROFILE\AppData\Local\nvim\init.vim" -Target "$env:HOMEPATH\dotfiles\nvim\init.vim" -Force

sudo New-Item -ItemType SymbolicLink -Path "$env:USERPROFILE\AppData\Local\Microsoft\PowerToys\settings.json" -Target "$env:HOMEPATH\dotfiles\PowerToys\settings.json" -Force
sudo New-Item -ItemType SymbolicLink -Path "$env:USERPROFILE\AppData\Local\Microsoft\PowerToys\Keyboard Manager\settings.json" -Target "$env:HOMEPATH\dotfiles\PowerToys\Keyboard Manager\settings.json" -Force
sudo New-Item -ItemType SymbolicLink -Path "$env:USERPROFILE\AppData\Local\Microsoft\PowerToys\Keyboard Manager\default.json" -Target "$env:HOMEPATH\dotfiles\PowerToys\Keyboard Manager\default.json" -Force
